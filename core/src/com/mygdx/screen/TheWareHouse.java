...
public class TheWareHouse extends AbstractScreen {
	...
	private Music m_run = Gdx.audio.newMusic(Gdx.files.internal("music/run.mp3"));
	private Music m_runbox = Gdx.audio.newMusic(Gdx.files.internal("music/run_box.mp3"));
	private Music Winner = Gdx.audio.newMusic(Gdx.files.internal("music/Winner.mp3"));
	...
	private void initial() {
		...
		levelButton = MenuCreator.createCustomGameButton(Assets.imgLbLevel, Assets.imgLbLevel);
		...
		btnUndo = MenuCreator.createCustomGameButton(Assets.btnMoveLeft, Assets.btnMoveLeft);
		btnUndo.setSize(35, 35);
		btnUndo.setPosition(OptionGame.SCREEN_WIDTH-220, OptionGame.SCREEN_HEIGHT-38);
...
//				}
			...
			private void moveBottomUndoBox() {
				for (int iBox = 0; iBox < lstBox.size(); iBox++) {
					...
					if (box.getPosX() == person.getPosX()-1
						...
						person.setAnimation(Assets.AnimPersonPushBottom, true, true);
						box.actionMoveTo(box.getPosY()*40, box.getPosX()*40, 0.5f);
						person.actionMoveTo(person.getPosY()*40, person.getPosX()*40, 0.5f);
						BottomPress = true;
						m_runbox.play();
						break;
					}
			...
			private void moveTopUndoBox() {
				for (int iBox = 0; iBox < lstBox.size(); iBox++) {
					...
					if (box.getPosX() == person.getPosX()+1
						...
						person.setAnimation(Assets.AnimPersonPushTop, true, true);
						box.actionMoveTo(box.getPosY()*40, box.getPosX()*40, 0.5f);
						person.actionMoveTo(person.getPosY()*40, person.getPosX()*40, 0.5f);
						TopPress = true;
						m_runbox.play();
			...
			private void moveLeftUndoBox() {
				for (int iBox = 0; iBox < lstBox.size(); iBox++) {
					...
					if (box.getPosX() == person.getPosX()
						...
						person.setAnimation(Assets.AnimPersonPushLeft, true, true);
						box.actionMoveTo(box.getPosY()*40, box.getPosX()*40, 0.5f);
						person.actionMoveTo(person.getPosY()*40, person.getPosX()*40, 0.5f);
						LeftPress = true;
						m_runbox.play();
			...
			private void moveRightUndoBox() {
				for (int iBox = 0; iBox < lstBox.size(); iBox++) {
					...
					if (box.getPosX() == person.getPosX()
						...
						person.setAnimation(Assets.AnimPersonPushRight, true, true);
						box.actionMoveTo(box.getPosY()*40, box.getPosX()*40, 0.5f);
						person.actionMoveTo(person.getPosY()*40, person.getPosX()*40, 0.5f);
						RightPress = true;
						m_runbox.play();
		...
		btnRedo = MenuCreator.createCustomGameButton(Assets.btnMoveRight, Assets.btnMoveRight);
		btnRedo.setSize(35, 35);
		btnRedo.setPosition(OptionGame.SCREEN_WIDTH-170, OptionGame.SCREEN_HEIGHT-38);
		getStage().addActor(btnRedo);
		
		
		btnRefresh = MenuCreator.createCustomGameButton(Assets.btnRefrect, Assets.btnRefrect);
		btnRefresh.setSize(35, 40);
		btnRefresh.setPosition(OptionGame.SCREEN_WIDTH-110, OptionGame.SCREEN_HEIGHT-36);
		...
		btnRefresh.addListener(new ActorGestureListener(){
			...
			public void touchUp(InputEvent event, float x, float y,
				...
				// TODO Auto-generated method stub
				super.touchUp(event, x, y, pointer, button);
				getGame().setScreen(new TheWareHouse(getGame(), "The WareHouse", level, currentLevel));
			}
		});
		
		btnvolume = new MenuCreator().createCustomGameButton(Assets.btnvolume, Assets.btnvolume);
		btnvolume.setPosition(OptionGame.SCREEN_WIDTH - 50, OptionGame.SCREEN_HEIGHT-36);
		...
		broad = new BroadGame();
		broad.setBroad(level.getMap(currentLevel), level.getWidth(currentLevel), level.getHeight(currentLevel));
	}
		...
		for (int i = 0; i < OptionGame.HEIGHT_BROAD; i++) {
			for (int j = 0; j < OptionGame.WIDTH_BROAD; j++) {
				if (OptionGame.WALL == broad.checkScreen(i, j)) {
					try {
						if (OptionGame.NULL != broad.checkScreen(i, j+1) 
								&& OptionGame.WALL != broad.checkScreen(i, j+1)) {
					...
					try {
						if (OptionGame.NULL != broad.checkScreen(i+1, j) 
								&& OptionGame.WALL != broad.checkScreen(i+1, j)) {
							...
							if (OptionGame.NULL != broad.checkScreen(i+1, j+1) && OptionGame.WALL != broad.checkScreen(i+1, j+1)) {
						...
						try {
							...
							// TODO: handle exception
							Gdx.app.log("ScreenLog", "EXCEPTION TOP LEFT: " +e2);
						}
						try {
							if (OptionGame.NULL != broad.checkScreen(i+1, j+1) && OptionGame.WALL != broad.checkScreen(i+1, j+1)) {
								Image wall = new Image(Assets.imgWallTopRight);
							...
							// TODO: handle exception
							Gdx.app.log("ScreenLog", "EXCEPTION TOP RIGHT: " +e2);
						}
					}
					try {
						if (OptionGame.NULL != broad.checkScreen(i-1, j) && OptionGame.WALL != broad.checkScreen(i-1, j)) {
							Image wall1 = new Image(Assets.imgWallBottomLeft);
							...
							if (OptionGame.NULL != broad.checkScreen(i-1, j+1) 
									&& OptionGame.WALL != broad.checkScreen(i-1, j+1)) {
								Image wall = new Image(Assets.imgWallBottomRight);
								wall.setPosition(j*40, i*40);
								getStage().addActor(wall);
							}
							if (OptionGame.NULL != broad.checkScreen(i-1, j-1) 
						...
						try {
							...
							Gdx.app.log("ScreenLog", "EXCEPTION BOTTOM LEFT: " + e2);
						}
						try {
							if (OptionGame.NULL != broad.checkScreen(i-1, j+1) 
									&& OptionGame.WALL != broad.checkScreen(i-1, j+1)) {
							...
							Gdx.app.log("ScreenLog", "EXCEPTION BOTTOM RIGHT: " + e2);
						}
					}
				...
				if (OptionGame.BOX == broad.checkScreen(i, j)
						|| OptionGame.PLACE_BOX == broad.checkScreen(i, j)) {
					BoxGame box = new BoxGame(Assets.imgBox, true, i, j, OptionGame.SIZE_WIDTH_ANIMATION, OptionGame.SIZE_HEIGHT_ANIMATION);
					lstBox.add(box);
				}
				if (OptionGame.PERSON == broad.checkScreen(i, j)
						|| OptionGame.PERSON_ON_GOAL == broad.checkScreen(i, j)) {
					person = new Person(i, j, OptionGame.SIZE_WIDTH_ANIMATION, OptionGame.SIZE_HEIGHT_ANIMATION);
	...
	public void keyBackPressed() {
		...
		getGame().setScreen(new LevelScreen(getGame(), "LEVEL SCREEN", level, currentLevel));
	}
	...
	public synchronized void render(float delta) {
		...
		if (processTouch && actionComplete) {
			if (!activeTouch) {
				activeTouch = false;
				if (Math.abs(lastPos_X-firstPos_X)<Math.abs(lastPos_Y-firstPos_Y)) {
					if (lastPos_Y > firstPos_Y) {
						MOVE_DOWN = true;
					} else if (lastPos_Y < firstPos_Y) {
						MOVE_TOP = true;
					}
				} else if (Math.abs(lastPos_X-firstPos_X)>Math.abs(lastPos_Y-firstPos_Y)) {
		...
		if (broad.checkComplete()) {
			...
			if (secondTime - firstTime > 3000000) {
				getGame().setScreen(new LevelScreen(getGame(), "MENU SCREEN", level, currentLevel + 1));
			}
		}
	}
